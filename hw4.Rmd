...
warming_predict <- warming %>% 
  mutate(damage_np = predict(damages.qm, list(warming = warming_baseline, warming2 = warming_baseline^2)), # damage without pulse
         ...
         wb_normalized = warming_baseline / warming_pulse[1], # normalize warming baseline with warming pulse value
         wp_normalized = warming_pulse / warming_pulse[1], # normalize pulse with first pulse value
         wbn_damage = predict(damages.qm, list(warming = wb_normalized, warming2 = wb_normalized^2)), # predict baseline 
         wpn_damage = predict(damages.qm, list(warming = wp_normalized, warming2 = wp_normalized^2)), # predict pulse
         difference_norm = wpn_damage - wbn_damage) # normalized difference
...
p4 <- ggplot(warming_predict) +
  geom_line(aes(x = year, y = difference_norm)) +
  theme_minimal()
